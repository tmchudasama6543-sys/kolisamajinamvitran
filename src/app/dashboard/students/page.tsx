...
export default function StudentsListPage() {
  ...
  const handleWhatsappClick = (e: React.MouseEvent, s: StudentData) => {
    e.preventDefault();
    const text = encodeURIComponent(whatsappTemplate.replace('{નામ}', s.name).replace('{મોબાઈલ}', s.mobileNumber));
    const phone = `91${s.mobileNumber}`;
    
    // Check user agent
    const ua = navigator.userAgent || navigator.vendor || (window as any).opera;
    
    if (/android/i.test(ua)) {
      // Android Intent: Forces the native app to open
      window.location.href = `intent://send?phone=${phone}&text=${text}#Intent;scheme=whatsapp;package=com.whatsapp;end`;
    } else {
      // iOS and Desktop: Use native custom scheme
                  ...
                  <TableRow key={s.id} className={cn("hover:bg-slate-50/50 transition-all border-b-2 last:border-none group", selectedIds.has(s.id) && "bg-slate-50")}>
                    ...
                    <TableCell className="p-6 text-right">
                      <div className="flex justify-end gap-2">
                         <Button onClick={(e) => handleWhatsappClick(e, s)} variant="outline" size="icon" className="h-12 w-12 rounded-xl border-2 border-green-500 text-green-600 hover:bg-green-50 transition-all shadow-sm" title="WhatsApp મેસેજ મોકલો">
