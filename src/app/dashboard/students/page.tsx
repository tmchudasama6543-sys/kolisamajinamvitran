                  ...
                  <TableRow key={s.id} className={cn("hover:bg-slate-50/50 transition-all border-b-2 last:border-none group", selectedIds.has(s.id) && "bg-slate-50")}>
                    ...
                    <TableCell className="p-6 text-right">
                      <div className="flex justify-end gap-2">
                         <a 
                           href={`https://wa.me/91${s.mobileNumber}?text=${encodeURIComponent(whatsappTemplate.replace('{નામ}', s.name).replace('{મોબાઈલ}', s.mobileNumber))}`} 
                           target="_blank" 
                           rel="noopener noreferrer"
                           className="flex items-center justify-center h-12 w-12 rounded-xl border-2 border-green-500 text-green-600 hover:bg-green-50 transition-all shadow-sm"
                           title="WhatsApp મેસેજ મોકલો"
                         >
